...
class HubServiceManager(object):
    ...
    def start(self, dmd, reactor):
        ...
        worklist = ZenHubWorklist(modeling_paused=modeling_paused)

        # configure Metrology for the worklists
        register_metrics_on_worklist(worklist)

        self.__workers = WorkerPoolDispatcher(reactor, worklist)
        events = EventDispatcher(dmd.ZenEventManager)
        executor = DispatchingExecutor([events], default=self.__workers)
    ...
    @property
    def worklist(self):
        return self.__workers.worklist
    ...
    def onExecute(self, listener):
        ...
        """
        self.__workers.onExecute(listener)

    @defer.inlineCallbacks
    def reportWorkerStatus(self):
        yield self.__workers.reportWorkerStatus()

    def getStatusReport(self):
        now = time.time()
        gauges, workTracker, execTimer = self.__workers.getStats()
...
class HubAvatar(pb.Avatar):
    ...
    def __init__(self, services, workers):
        ...
        @param workers {WorkerPoolDispatcher} Executes jobs on zenhubworkers.
        """
...
# Note: The name 'WorkerInterceptor' required to remain compatible with the
# EnterpriseCollector zenpack.
