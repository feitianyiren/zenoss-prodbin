...
from Products.ZenHub.dispatchers import ExecutionStats, WorkerStats
...
class HubServiceManagerTest(TestCase):
    ...
    def test_start(
        ...
        workerPoolDispatcher.assert_called_once_with(reactor, worklist)
        ...
        hubAvatar.assert_called_once_with(services, workers)
    ...
    @patch("{src}.WorkerPoolDispatcher".format(**PATH), autospec=True)
    def test_getStatusReport(self, workers):
        gauges = {
        ...
        workers.getStats.return_value = (gauges, workTracker, execTimer)
        manager = HubServiceManager(
            modeling_pause_timeout=self.modeling_pause_timeout,
            passwordfile=self.passwordfile,
            pbport=self.pbport,
        ...
        manager._HubServiceManager__workers = workers
...
class HubAvatarTest(TestCase):
    ...
    @patch("{src}.logging".format(**PATH), name="logging", autospec=True)
    def test_perspective_getService_raises_error(self, logging):
